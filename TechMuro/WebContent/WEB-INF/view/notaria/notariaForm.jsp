	...
	$(document).ready(function() {
		...
		grid = $("#listDetalleNotaria");
	            ...
	            grid.jqGrid({
					...
					colModel:[
					    {name : 'idcontrato', index : 'name', sorttype : "int", align : "center", viewable : false, hidden : true}, 
						{name:'descripcion',index:'descripcion', width:280, sorttype:"int", align:"left"},
						{name:'gastos',index:'gastos', width:200, sorttype:"int", align:"center"}, 
			...
			<div class="box-2" id="box">
				...
				<c:choose>
				    <c:when test="${!empty notaria.idnotaria}">
				        <div id="title-header-popup">Editar Notaría</div>
				    </c:when>
				    <c:otherwise>
				        <div id="title-header-popup">Agregar Notaría</div>
				    </c:otherwise>
				</c:choose>
				...
				<form:form method="post" action="../notaria/agregar.htm" commandName="notaria" 
					...
					<ul id="body-popup-2">
						...
						<li>Email:</li>
						<li id="right-li"><form:input path="email" id="email" type="email"
							class="validate[custom[email]] input-popup-2" maxlength="100"/></li>
						<li>Plaza:</li>
						<li id="right-li">
