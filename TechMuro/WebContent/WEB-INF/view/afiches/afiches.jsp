	...
	<script>
		...
		$(document).ready(function () {
			...
			'<c:forEach items="${lisAll}" var="a">';
				hh.push({idoficina:'${a.oficina.idoficina}',idafiche:'${a.afiche.idafiche}',imagen:'0',oficina:'${a.oficina.codigo}',descripcion:'${a.afiche.descripcion}',miniatura:'${a.afiche.miniatura}'} );
		    '</c:forEach>';
			llenarGrid(hh);
	    ...
	    function linkUp(){
	     	$("#up").load("../upAgregarAfiche.htm", function(){
	     ...
	     function llenarGrid(Json){
	            ...
	            grid.jqGrid({
	                datatype:'local',
	                data: Json,
	               	colNames: ["idoficina","N°", "Afiche", "Oficina", "Descripción", "Miniatura", "Editar", "Eliminar"],
					colModel:[ { name : 'idoficina', index : 'name', sorttype : "int", align : "center", viewable : false, hidden : true},
						{name:'idafiche',index:'nro', width:54, sorttype:"int", align:"center"},
						{name:'afiches',index:'invdate', width:84, sorttype:"int", align:"center"}, 
						{name:'oficina',index:'name', width:84, align:"center"},
						{name:'descripcion',index:'name', width:216, align:"center"},
						{name:'miniatura',index:'tax', width:79, align:"center",sorttype:"float"},
